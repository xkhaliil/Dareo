...
async function updateUserXp(userId: string, delta: number) {
  ...
  return prisma.user.update({
    ...
    data: {
      ...
      rank: computeRank(newXp) as Rank,
    },
  });
}
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const xpCaps: Record<string, number> = {
        EASY: 25,
        MEDIUM: 50,
        HARD: 100,
        EXTREME: 200,
      ...
      const clampedXp = Math.min(
        Math.max(typeof xpReward === "number" ? xpReward : 10, 1),
        cap,
      );
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (xpReward !== undefined)
        data.xpReward = Math.min(Math.max(Number(xpReward) || 10, 1), cap);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!status || !validStatuses.includes(status)) {
        res
          .status(400)
          .json({ error: "Status must be COMPLETED, PASSED, or FAILED" });
        return;
      }
